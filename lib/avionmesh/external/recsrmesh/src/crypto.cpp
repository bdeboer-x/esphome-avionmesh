...
{
    uint8_t digest[backend::SHA256_LEN];
    ByteSpan pass_span{};
    if (passphrase && *passphrase)
        pass_span = {reinterpret_cast<const uint8_t *>(passphrase), std::strlen(passphrase)};
    ByteSpan salt_span{};
    if (salt && salt_len > 0)
        salt_span = {reinterpret_cast<const uint8_t *>(salt), salt_len};

    if (backend::sha256(pass_span, salt_span, digest) != Error::Ok)
        return Error::Crypto;
...
// HMAC-SHA256 (RFC 2104) built on the backend's SHA-256, so it works with
// any key length (including empty) on every Mbed TLS generation.
Error hmac_sha256(ByteSpan data, ByteSpan key, uint8_t out[HMAC_LEN])
{
    ...
    if (key.size() > BLOCK) {
        if (backend::sha256(key, key_block) != Error::Ok)
            return Error::Crypto;
    } else if (!key.empty()) {
    ...
    uint8_t inner[backend::SHA256_LEN];
    if (backend::sha256({pad, BLOCK}, data, inner) != Error::Ok)
        return Error::Crypto;

    for (size_t i = 0; i < BLOCK; i++)
        pad[i] = key_block[i] ^ 0x5c;

    if (backend::sha256({pad, BLOCK}, {inner, sizeof(inner)}, out) != Error::Ok)
        return Error::Crypto;
...
uint32_t uuid_to_hash31(ByteSpan uuid_bytes)
{
    ...
    uint8_t digest[32] = {};
    backend::sha256(uuid_bytes, digest);
...
uint64_t uuid_to_hash64(ByteSpan uuid_bytes)
{
    ...
    uint8_t digest[32] = {};
    backend::sha256({reversed, len}, digest);
